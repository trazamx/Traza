...
app.post('/create-payment-intent', async (req, res) => {
  ...
  if (!STRIPE_SECRET_KEY)
    return res.status(500).json({ ok: false, error: 'STRIPE_SECRET_KEY no configurada' });
...
app.post('/chat', async (req, res) => {
  ...
  if (Array.isArray(history))
    history.slice(-6).forEach(m => { if (m.role && m.content) messages.push({ role: m.role, content: m.content }); });
  ...
  try {
    ...
    const payload = JSON.stringify({
      ...
      max_tokens: 400,
      system: 'Eres el asistente virtual de TRAZA Logistica, empresa de entregas en Monterrey. Responde en espanol, conciso. Express $149, Same-Day $99, Programado $199. Tel: 811 555 0619.',
      messages
    });
    const options = {
...
app.get('/dashboard', (req, res) => {
  res.send(`<!DOCTYPE html><html><head><title>TRAZA Bridge</title><meta charset="UTF-8">
<style>body{font-family:monospace;background:#0a0a0f;color:#f2f2f8;padding:24px}h1{color:#2d7eff}a{color:#2d7eff;margin-right:18px}button{background:#2d7eff;color:#fff;border:none;padding:10px 22px;border-radius:8px;cursor:pointer;margin-top:14px}</style>
</head><body>
<h1>TRAZA Bridge</h1>
<p id="s">Loading…</p>
<a href="/app">App</a><a href="/vehicles">Vehicles</a><a href="/status">Status</a><br>
<button onclick="force()">Force Sync</button>
<script>
async function load(){const d=await fetch("/status").then(r=>r.json()).catch(()=>({}));document.getElementById("s").textContent="Status: "+d.syncStatus+" | Vehicles: "+(d.vehicleCount||0);}
...
async function login() {
  ...
  try {
    ...
    const filled = await page.evaluate((user, pass) => {
      ...
      setter.call(userInput, user); userInput.dispatchEvent(new Event('input', { bubbles: true }));
      setter.call(passInput, pass); passInput.dispatchEvent(new Event('input', { bubbles: true }));
      return { ok: true };
    ...
    await page.evaluate(() => {
      ...
      if (btn) btn.click(); else { const form = document.querySelector('form'); if (form) form.submit(); }
    });
...
async function pollVehicles() {
  ...
  try {
    ...
    if (domVehicles?.length > 0) {
      ...
      if (!page.url().includes('login')) await page.reload({ waitUntil: 'networkidle2', timeout: 15000 }).catch(() => {});
      else await login();
    }
...
async function start() {
  app.listen(PORT, '0.0.0.0', () => {
    console.log('TRAZA Bridge corriendo en puerto ' + PORT);
    console.log('   App:       /app');
  ...
  if (!SPHEREGT_USER || !SPHEREGT_PASS) {
    console.warn('WARNING: GPS bridge desactivado — configura SPHEREGT_USER / SPHEREGT_PASS en Railway.');
    syncStatus = 'waiting';
    return;
  }
  await launchBrowser();
  const ok = await login();
  if (ok) { await pollVehicles(); setInterval(pollVehicles, POLL_INTERVAL); console.log('Polling cada ' + (POLL_INTERVAL / 1000) + 's'); }
}
